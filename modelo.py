...
for key, value in dict_var_rad_esp.items():
    ...
    if (key != 'arquero') and ('xG per shot taken' in dict_var_rad[key]):
        ...
        s.add('xG per shot')
        dict_var_rad[key] = s
dict_translate_players_reverse = {value:key for key, value in dict_translate_players.items()}
dict_translate_gk_reverse = {value:key for key, value in dict_translate_gk.items()}
...
def radar_streamlit_escoger(df_radar, df_raw, df_tabla, position, w, variables):
    ...
    #variables = st.multiselect('Seleccione las variables que quiere observar',options=v_esp)
    #if 'arquero' not in position:
